...
class _FaceDetectionPageState extends State<FaceDetectionPage> {
  ...
  void _handleAddAnchor(ARKitAnchor anchor) {
    ...
    helmet = _createHelmet(faceAnchor.leftEyeTransform);
    arkitController.add(helmet, parentNodeName: anchor.nodeName);
  }

  ARKitNode _createHelmet(Matrix4 transform) {
    final position = vector.Vector3(
      transform.getColumn(3).x - 0.036,
      transform.getColumn(3).y + 0.045,
      transform.getColumn(3).z - 0.08,
    );

    var helmet = ARKitReferenceNode(
      url: 'models.scnassets/model.dae',
      position: position,
      scale: vector.Vector3(0.17, 0.17, 0.17),
      renderingOrder: -1,
    );
    return helmet;
  }
